...
class ImageNetExperiment():
    ...
    def setup_params(self):
        ...
        def loss_func(inputs, outputs):
            return tf.nn.sparse_softmax_cross_entropy_with_logits(labels=outputs['gt_Q'], 
                                                                  logits=outputs['pred'])
    ...
    def validation_loss(self, inputs, outputs):
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=outputs['gt_Q'], 
                                                               logits=outputs['pred'])
        return {'loss': loss } 
    ...
    def in_top_k(self, inputs, outputs):
        ...
        """
        return {'top1': tf.nn.in_top_k(outputs['pred'], outputs['labels'], 1),
                'top5': tf.nn.in_top_k(outputs['pred'], outputs['labels'], 5)}
