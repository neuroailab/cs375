...
from models_colorization import colorization_model
...
class ImageNetClassificationExperiment():
    ...
    class Config():
        ...
        """
        target_layers=[ 'bn' + str(layer) for layer in range(1, 8)] + ['conv8_3', 'pred']
    ...
    def __init__(self, exp, step, batch, crop):

        self.Config.exp_id = exp
        self.Config.batch_size = batch
        self.Config.crop_size = crop
        self.Config.extraction_step = step
        assert ImageNetDataProvider.N_VAL % batch == 0, \
                ('number of examples not divisible by batch size!')
        self.Config.val_steps = int(ImageNetDataProvider.N_VAL / batch)
        self.feature_masks = {}
...
if __name__ == '__main__':
    ...
    for iteration in [40000]:
        print("Running cifar model at step %s" % iteration)
        base.get_params()
        m = ImageNetClassificationExperiment('exp_reg', iteration, 50, 224)
